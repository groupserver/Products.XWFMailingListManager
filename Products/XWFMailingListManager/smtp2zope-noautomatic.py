...
# Determine list properties automatically, or not
AUTOMATIC = 0
...
def getAuthorizedURL(url, auth):
    # annoyingly, xmlrpclib only recognises user@pass style
    # authentication, so having gone to the trouble of parsing
...
if AUTOMATIC:
    ...
    if not list_props:
        ...
        sys.exit(EXIT_NOPERM)
    baseURL = '%s/%s/' % (callURL, list_props['id'])
    callURL = '%smanage_mailboxer' % (baseURL)
    max_size = list_props.get('max_size', DEFAULT_MAX_SIZE)
else:    
...
if len(sys.argv) == 3:
    ...
    maxBytes  = maxBytesOverride or DEFAULT_MAX_SIZE
...
if maxBytes>0 and mailLen>maxBytes:
    log_warning('Rejecting email, due to size (%s bytes, limit %s bytes)' %
                (mailLen, maxBytes))
    event_codes.append(102) # email too long
