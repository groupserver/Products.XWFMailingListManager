...
class WebPostAuditEventFactory(object):
    ...
    title = 'GroupServer Web Post Audit Event Factory'
    description = 'Creates a GroupServer audit event for web posting'
    ...
    def __call__(self, context, event_id, code, date,
        ...
        if (code == POST):
            event = PostEvent(context, event_id, date, userInfo, siteInfo,
...
class PostEvent(BasicAuditEvent):
    ...
    def __str__(self):
        ...
        """
        retval = '%s (%s) making a post from the web to the '\
                 'topic "%s" in %s (%s) on %s (%s)' %\
